// Renderizador del tablero. Dibuja fondo, nodos, aristas, componentes,
// preview de rutas, animacion de corriente y textos flotantes sobre el canvas.
...
export class Renderer {
  ...
  drawBackground () {
    ...
    const gridSize = 48;
    const scrollOffset = Math.floor (t * 8) % gridSize;
    ctx.strokeStyle = CONFIG.COLORS.WIRE + '88';
    ctx.lineWidth = 1;
  ...
  drawBoard (board) {
    ...
    if (
      this.showPreview &&
      this.previewResult &&
      this.previewResult.route.length > 1
    ) {
  ...
  drawNodes (board) {
    ...
    for (const node of board.nodes) {
      ...
      // Esquinas decorativas (doble pixel)
      ...
      if (node.type === NODE_SOURCE) {
        ...
        if (comp) {
          ...
          const label = comp.getLabel ? comp.getLabel (node) : comp.name;
        ...
        ctx.fillStyle = blink2
          ? CONFIG.COLORS.ACCENT_YELLOW + 'cc'
          : CONFIG.COLORS.ACCENT_YELLOW + '55';
  ...
  drawComponentEffect (ctx, board, node, S) {
    ...
    switch (comp.ringStyle) {
      ...
      case 'invert': {
        // Borde dashed pulsante + etiqueta con el estado del "armado".
        // Si hay dado colocado: muestra si esta en MAX (verde) o no (rojo).
        // Si no hay dado: muestra "MAX?" en el color del componente.
        const alpha = pulse ? 'ff' : '66';
        ctx.strokeStyle = comp.color + alpha;
        ctx.lineWidth = 2;
        ctx.setLineDash ([3, 3]);
        ...
        let label = 'MAX?';
        let labelColor = comp.color;
        if (node.dieFaces != null && node.dieValue != null) {
          const atMax = node.dieValue === node.dieFaces;
          label = atMax ? 'x2!' : `${node.dieValue}/${node.dieFaces}`;
          labelColor = atMax
  ...
  animateRoute (board, route, scoringResult, onComplete, onScoreUpdate = null) {
    ...
    // Precalcular score acumulado por nodo
    const nodeScores = {};
    let running = 0;
    ...
    const duration = 500 + totalSegments * 350;
    ...
    const animate = now => {
      ...
      if (currentSeg > lastSegment && currentSeg < totalSegments) {
        ...
        if (to.dieValue !== null && toScore) {
          ...
          const burstCount = clamp (
            6 + Math.floor (this._routeRunningScore / 3) * 3,
            6,
            30
          );
          if (this.juice) {
            ...
            if (toScore.value >= 5) {
              this.juice.spawnNodeBurst (
                ...
                to.y,
                '#ffffff',
                Math.floor (burstCount / 3)
              );
            }
            this.juice.shake (
              2 + Math.floor (this._routeRunningScore / 8),
              100
            );
            if (toScore.value >= 8) {
          ...
          if (this.audio) {
            this.audio.playNodeScore (
              toScore.runningTotal,
              this._routeBaseScore || 30
            );
          }
        } else {
          if (this.audio) {
      ...
      if (this.juice && this.animProgress > 0.05 && this.animProgress < 0.98) {
        const segIdx2 = Math.min (
          Math.floor (this.animProgress * totalSegments),
          totalSegments - 1
        );
        const segFrac2 = this.animProgress * totalSegments - segIdx2;
        const f2 = board.getNode (route[segIdx2]);
        const t2 = board.getNode (
          route[Math.min (segIdx2 + 1, route.length - 1)]
        );
        ...
        this.juice.spawnTrail (
          ...
          hp.y,
          CONFIG.COLORS.ACCENT_CYAN,
          1 + Math.floor (this.animProgress * 3)
        );
      }
  ...
  drawActiveRoute (board, route, progress) {
    ...
    const glowAlpha = Math.floor (lerp (0x44, 0xcc, intensity))
      .toString (16)
      .padStart (2, '0');
  ...
  drawTooltip (board) {
    ...
    const extra = comp.getTooltipExtra ? comp.getTooltipExtra (node) : null;
