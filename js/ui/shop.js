// === TIENDA CON PESTANAS: COMPONENTES / DADOS / COFRES ===
...
class Shop {
    ...
    render() {
        ...
        // Display de dinero en la tienda
    ...
    openChest(chest) {
        ...
        rewards.forEach((reward, i) => {
            ...
            setTimeout(() => {
                ...
                if (this.juice) {
                    // Particulas en la posicion del card
    ...
    generateRewards(count, guaranteeRare = false) {
        ...
        const pool = [
            ...
            // Dinero
            { weight: 5,  gen: () => ({ type: 'money', amount: randInt(3, 8), name: `+$`, desc: 'Dinero extra', symbol: '$$', color: '#ffd700' }) },
            // Vida
            { weight: 2,  gen: () => ({ type: 'life', name: '+1 VIDA', desc: 'Una oportunidad mas', symbol: '<3', color: '#ff3a3a' }) },
            // Upgrade dado aleatorio
            { weight: 3,  gen: () => ({ type: 'upgrade', name: 'UPGRADE', desc: 'Mejora tu peor dado', symbol: '^^', color: '#39ff14' }) },
        ];
        ...
        if (guaranteeRare && !rewards.some(r => r.type === 'die' && ['d10', 'd12'].includes(r.dieType))) {
    ...
    applyReward(reward) {
        switch (reward.type) {
            ...
            case 'life':
                ...
                document.getElementById('lives-display').textContent = '\u2665'.repeat(this.playerState.lives);
                break;
            case 'upgrade':
                const order = ['d4', 'd6', 'd8', 'd10', 'd12'];
                let bestIdx = -1;
    ...
    enterPlacementMode() {
        ...
        this._canvasMoveHandler = (e) => {
            ...
            this.board.nodes.forEach(n => {
                n.hover = this.isPlaceableNode(n) && dist(pos.x, pos.y, n.x, n.y) <= CONFIG.BOARD.NODE_RADIUS * 1.3;
            });
            this.renderer.drawBoard(this.board);
        };
    ...
    updateMoney() {
        ...
        // Actualizar el display de dinero dentro de la tienda
