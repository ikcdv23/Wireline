...
class Game {
    constructor() {
        ...
        this.input = new InputHandler(
            this.canvas, this.board, this.diceManager,
    ...
    goToMenu() {
        ...
        // Mostrar menu principal y arrancar musica
        setTimeout(() => {
    ...
    activateCircuit() {
        ...
        // Animar corriente con scoring en vivo — actualizar HUD en tiempo real
        const baseRoundScore = this.roundScore;
        this.renderer.animateRoute(this.board, result.route, result, () => {
            ...
            // Mostrar breakdown con animacion y sonido
            ...
            if (totalLive > this.state.target * 1.5) {
                // Emitir particulas desde la zona del score en el HUD
    ...
    resolveActivation(score) {
        ...
        if (reachedTarget) {
            ...
            // Consolacion: ganas $1 por cada nodo con dado que la corriente toco
            ...
            if (this.state.lives <= 0) {
                ...
                // Reiniciar la MISMA ronda, no avanzar
                this.renderer.addFloatingText(
                    400, 320,
                    'REINTENTAR RONDA',
                    CONFIG.COLORS.TEXT_DIM
    ...
    runEndOfRound() {
        for (const node of this.board.nodes) {
            ...
            if (node.component && node.component.usesLeft !== undefined && node.component.usesLeft <= 0) {
                node.component = null;
            }
        }
    }
    ...
    advanceRound() {
        ...
        if (showShop) {
            ...
            this.hud.update(this.state);
            this.shop.open(this.state, this.board, this.renderer, this.audio, this.juice, this.diceManager, () => {
                this.startNewRound();
            });
        } else {
            this.startNewRound();
    ...
    showScoreBreakdown(result) {
        ...
        // Acumulado en la ronda si hay multi-tiro
