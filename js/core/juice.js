// === JUICE: SCREENSHAKE, PARTICULAS, EFECTOS VISUALES ===
...
class JuiceEngine {
    ...
    shake(intensity = 5, duration = 200) {
        ...
        const currentDecay = elapsed < this.shakeDuration ? (1 - elapsed / this.shakeDuration) * this.shakeIntensity : 0;
    ...
    spawnParticles(x, y, count, color, opts = {}) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            this.particles.push({ el: p, x: screenX, y: screenY, vx, vy, life, maxLife: life, born: performance.now() });
        }
    ...
    // Particulas de electricidad a lo largo de un segmento
    ...
    // VICTORIA de ronda — explosion masiva en cascada
    spawnVictoryExplosion(x, y) {
        ...
        // Lluvia de confeti por toda la pantalla
        setTimeout(() => this.spawnConfetti(), 150);
    }

    // Confeti — particulas de colores por toda la pantalla
    spawnConfetti(count = 40) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            this.particles.push({ el: p, x: sx, y: top - 20, vx, vy, life, maxLife: life, born: performance.now(), noGravity: true, spin: (Math.random() - 0.5) * 10 });
        }
        if (!this._particleLoopRunning) {
            this._particleLoopRunning = true;
            this._animateParticles();
    ...
    // GAME OVER — particulas rojas descendentes
    spawnGameOverEffect() {
        for (let i = 0; i < 5; i++) {
            setTimeout(() => {
                const cx = 400;
    ...
    // Spark trail — particulas que persiguen un punto
